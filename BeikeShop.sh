...
echo "[4/8] 写入 Nginx 配置..."
cat <<NGX > /etc/nginx/sites-available/beikeshop
server {
    listen 80;
    server_name ${DOMAIN};
...
echo "[5/8] 申请 HTTPS 证书..."
certbot --nginx -d ${DOMAIN} --non-interactive --agree-tos -m ${EMAIL}

echo "[6/8] 写入 HTTPS 配置..."
...
echo "[7/8] 开放防火墙（如启用 UFW）..."
ufw allow 'Nginx Full' || true

echo "[8/8] ✅ BeikeShop 安装完成，请访问：https://${DOMAIN}"
echo "---"
echo "数据库配置：${DB_NAME} / ${DB_USER} / ${DB_PASS}"
echo "如需正式收款，请登录后台启用 PayPal 插件并填写 Key"
